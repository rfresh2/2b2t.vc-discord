    ...
    private WebhookMessageCreateAction<Message> handleChatWatch(final SlashCommandInteractionEvent event, final String sub) {
        ...
        switch (sub) {
            case "add" -> {
                ...
                String keyword = Optional.ofNullable(event.getOption("keyword", OptionMapping::getAsString))
                    .orElse("")
                    .replace("< >", " ");
                ...
                boolean caseSensitive = Optional.ofNullable(event.getOption("case-sensitive",
                ...
                return event.getHook().sendMessageEmbeds(embed(event)
                    ...
                    .setDescription("Notifications on chats containing `%s` will be sent to: %s".formatted(keyword,
            ...
            case "clear" -> {
                ...
                return event.getHook().sendMessageEmbeds(embed(event)
                    ...
                    .setDescription("Removed %d watches.\n".formatted(watches.size()) + watches.stream()
    ...
    private WebhookMessageCreateAction<Message> handlePlayerWatch(final SlashCommandInteractionEvent event, final String sub) {
        ...
        switch (sub) {
            case "add" -> {
                ...
                boolean leaves = Optional.ofNullable(event.getOption("leaves", OptionMapping::getAsBoolean))
                    .orElse(true);
                boolean chats = Optional.ofNullable(event.getOption("chats", OptionMapping::getAsBoolean)).orElse(true);
                boolean deaths = Optional.ofNullable(event.getOption("deaths", OptionMapping::getAsBoolean))
                    .orElse(true);
                ...
                return event.getHook().sendMessageEmbeds(populateIdentity(embed(event), p)
                    ...
                    .setDescription("Notifications on `%s` events for `%s` will be sent to: %s".formatted(eventsList,
                        p.name(),
                        channel.getAsMention()))
            ...
            case "delete" -> {
                var playerName = Optional.ofNullable(event.getOption("player", OptionMapping::getAsString))
                    .map(String::trim)
                    .orElse("");
